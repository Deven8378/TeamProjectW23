...
class Message extends \app\core\Model{
	...
	public function getSent($user_id){
		$SQL = "SELECT `message`.`message_id`, `message`.`message`, `message`.`timestamp`, sendertable.`email` AS `sender_email`, sendertable.`first_name` AS `sender_fname`, sendertable.`last_name` AS `sender_lname` FROM `message` JOIN `profile` AS `sendertable` ON `message`.`receiver` = sendertable.`user_id` WHERE sender=:sender;";
		$STH = $this->connection->prepare($SQL);
		$data = ['sender'=>$user_id];
		$STH->execute($data);
		$STH->setFetchMode(\PDO::FETCH_CLASS, 'app\\models\\Message');
