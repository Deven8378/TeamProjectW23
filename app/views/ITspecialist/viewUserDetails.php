...
<div class="userDetails">
    ...
    <div class="alert" style="background-color: rgb(216,212,212);">
  		<h3><?= htmlentities($data->username) ?> (<?= htmlentities($data->user_id) ?>)</h3>
    </div>
    ...
    <dl class="dl-viewUserDetails" style="">
      	...
      	<div class="" style="flex: 70%; " >
      		<!-- col-sm-3 -->
	            	...
	            	<dd class="dd-left"><?= htmlentities($data->first_name) ?></dd>

	            	<dt class="dt-label"><?=_('Middle Name')?></dt>
	            	<dd class="dd-left"><?= htmlentities($data->middle_name) ?></dd>

	            	<dt class="dt-label"><?=_('Last Name')?></dt>
	            	<dd class="dd-left"><?= htmlentities($data->last_name) ?></dd>

	            	<dt class="dt-label"><?=_('Email')?></dt>
	            	<dd class="dd-left"><?= htmlentities($data->email)?></dd>

	            	<dt class="dt-label"><?=_('Phone Number')?></dt>
	            	<dd class="dd-left"><?= htmlentities($data->phone_number) ?></dd>

	            	<dt  class="dt-label"><?=_('Status')?></dt>
	            	<dd  class="dd-left"><?= htmlentities($data->status) ?></dd>
		...
		<div class="" style="flex: 20%;" >
			...
			<div style="" >
				<div style=" " >
	        		...
	        		<dd class="dd-right" style=""><?= htmlentities($data->user_id) ?></dd>
	        	</div>
	        	<div >
	        		<dt class="dt-label"><?=_('Username')?></dt>
	        		<dd class="dd-right"><?= htmlentities($data->username)?></dd>
	        	</div>
	        	<div >
	        		<dt  class="dt-label"><?=_('Password')?></dt>
	        		<dd  class="dd-right"><?= htmlentities($data->password_hash) ?></dd>
	        	</div>
	        </div>
            ...
            <div class="content">
                <div class="container">
                    <form  method="post" action="">
                        ...
                        <a href="/ITspecialist/deleteUser/<?= htmlentities($data->user_id) ?>"><?=_('delete')?></a>
                    </form>
                </div>
            </div>
        </div>
